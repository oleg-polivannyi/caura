...
# The Polza deepseek route can take ~84s on large content; the code default is
# 25s (common/llm/constants.py OPENAI_REQUEST_TIMEOUT_SECONDS). Extraction runs
# as a background task, so this does not block the write response.
kubectl -n "$NS" set env deployment/"$DEPLOY" OPENAI_REQUEST_TIMEOUT_SECONDS=120
